...
\mcmSubsection{问题二：求解测线方向角为$\beta$时的覆盖宽度}

\mcmSubsubsection{测坡角公式}
...
其中$d$为测量船与中心海域的水平距离，$D(0)$是中心海域的深度。$\theta$为多波束换能器的开角，$\gamma$为测坡角.

将$\theta = 120^\circ$，$\gamma = 1.5^\circ$，$D(0) = 120m$代入上述公式，计算得出结果（详见源码question2.py）。
...
\mcmSubsection{问题三：为海域设计测线方案}

\mcmSubsubsection{确定测线的方向角$\beta$}
...
在该测线方案下，当满足模型假设情况时，该测线方案可完全覆盖所划海域，并可做到整体测量长度最短。测量长度为：132,988.49米。
